        ...
        try:
            with open(CACHE_FILE, "r") as f:
                ...
                if loaded_stocks and isinstance(loaded_stocks, list):
                    if "BID_VOL" not in loaded_stocks[0]: return [], None
...
# ==========================================
QUOTES_DATABASE = [
    {"quote": "Membeli di saat merah (support) dan menjual saat euphoria (top gainer) adalah hukum alam institusi.", "author": "Smart Money Concept", "theme": "Bottom Fishing"},
    {"quote": "Ketika antrean beli 2x lipat dari antrean jual, bandar sedang melindungi areanya. Ikuti arusnya.", "author": "Order Book Logic", "theme": "Tape Reading"},
...
# ==========================================
# 3. CORE ENGINE DATA FETCHING & ORDER BOOK ALGO
# ==========================================
MASTER_UNIVERSE = [
...
def fetch_single_stock(emiten, mode_tf):
    try:
        ...
        # --- ORDER BOOK & REACTIVE LOGIC (v16.5) ---
        info = yf.Ticker(emiten).info or {}
        bid_price = info.get('bid', 0)
        ask_price = info.get('ask', 0)
        bid_vol = info.get('bidSize', 0)
        ask_vol = info.get('askSize', 0)

        obi_ratio = 1.0
        orderbook_status = ""
        if bid_vol > 0 and ask_vol > 0:
            obi_ratio = bid_vol / ask_vol
            if obi_ratio >= 1.5: orderbook_status = " 🛡️ BUY WALL"
            elif obi_ratio <= 0.5: orderbook_status = " 🧱 SELL WALL"
        ...
        # Inject OBI into Bandarmologi Judgment
        if (is_near_support and is_hammer) or (is_near_support and obi_ratio >= 1.5): s_bandar = "🎯 SEROK (REJECTION/WALL)"
        elif is_near_support and is_vol_dry: s_bandar = "🔍 VOLUME DRY-UP (PANTAU)"
        elif is_fake_accum or obi_ratio <= 0.5: s_bandar = "⚠️ AWAS GUYURAN (FAKE/SELL WALL)"
        elif is_spike:
            if l_shadow > (b_size * 1.5): s_bandar = "🐋 AKUMULASI DASAR"
            elif u_shadow > (b_size * 1.5): s_bandar = "🩸 DISTRIBUSI PUCUK"
            elif is_bull and wpi_score > 70: s_bandar = f"🚀 MARK-UP BERINGAS{orderbook_status}"
        ...
        elif "AWAS" in s_bandar: grade = "⚠️ SETUP C (AVOID)"
        ...
        return {
            ...
            "BID": bid_price, "OFFER": ask_price, "BID_VOL": bid_vol, "OFFER_VOL": ask_vol
        }
    except: return None
...
def render_cross_validation_ui(active_tickers_tuple, market_climate_mult, is_trading_mode):
    ...
    <div style="margin-top: 10px; margin-bottom: 15px; padding-left: 10px; border-left: 4px solid #00f2fe;">
        ...
        <p style="color: #64748b; font-size: 0.8rem; font-weight: 400; margin-top: 2px;">{'Pilih target untuk membedah Momentum & Titik Reaksi.' if is_trading_mode else 'Bedah Laporan Keuangan Riil Kuartalan (Compact View).'}</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if active_tickers_tuple:
        ...
        if raw_target:
            if is_trading_mode:
                ...
                elif "AVOID" in setup_grade: sys_rec, color, r_mult = "AVOID (FAKE/SELL WALL)", "#f43f5e", 0.0
                ...
                with c2:
                    wpi_color = '#f43f5e' if ("FAKE" in s_bandar or "AWAS" in s_bandar) else ('#d4af37' if "SEROK" in s_bandar else ('#10b981' if wpi>70 else '#facc15' if wpi>40 else '#f43f5e'))
                    wpi_label = "REJECTION / BUY WALL" if "SEROK" in s_bandar else ("DISTRIBUTION / SELL WALL" if ("FAKE" in s_bandar or "AWAS" in s_bandar) else "WHALE PRESSURE")
...
with st.sidebar:
    ...
    st.markdown("<p style='color: #00f2fe; font-size: 0.65rem; letter-spacing: 2px; margin-bottom: 25px;'>v16.5 ORDER BOOK ENGINE</p>", unsafe_allow_html=True)
    ...
    if btn_ph.button("🔄 SCAN MARKET", use_container_width=True) or tf_berubah:
        ...
        my_bar = st.progress(0, text="Menyiapkan Radar Reaktif & Order Book...")
        dyn_tickers = get_dynamic_market_roster()
        for i, t in enumerate(dyn_tickers):
            my_bar.progress((i + 1) / len(dyn_tickers), text=f"Scanning Flow & Bid-Ask {t} ({i+1}/{len(dyn_tickers)})")
...
if not st.session_state.scan_clicked or not st.session_state.raw_stocks:
    st.info("👈 Tekan tombol '🔄 SCAN MARKET' untuk memulai Inisiasi Reactive Engine v16.5.")
else:
    h_trd, h_inv = [], []
    c_val, c_gro, c_div = [], [], []
    ...
    for r in st.session_state.raw_stocks:
        ...
        bid_v, ask_v = r.get("BID_VOL", 0), r.get("OFFER_VOL", 0)
        
        # Display Bid/Offer with Volume if available (E.g. "1.350 [200]")
        bid_str = f"{int(bid_p):,} [{int(bid_v)}]" if bid_p > 0 and bid_v > 0 else (f"{int(bid_p):,}" if bid_p > 0 else "-")
        offer_str = f"{int(ask_p):,} [{int(ask_v)}]" if ask_p > 0 and ask_v > 0 else (f"{int(ask_p):,}" if ask_p > 0 else "-")
        
        # Logika Prioritas Reactive & Order Book
        if "REACTIVE" in setup_grade: kep_t, prio = "🎯 SEROK BAWAH", 5
        elif "A+" in setup_grade: kep_t, prio = "🚀 STRONG ACCUM", 4
        elif "AGGRESSIVE" in setup_grade: kep_t, prio = "⚡ AGGRESSIVE SCALP", 3
        elif "B" in setup_grade: kep_t, prio = "🟢 ACCUMULATE", 2
        elif "AVOID" in setup_grade: kep_t, prio = "⚠️ AVOID (FAKE)", 0
        else: kep_t, prio = "🟡 HOLD", 1
        
        # Boost/Downgrade priority based on Order Book Imbalance (jika data API ada)
        if bid_v > 0 and ask_v > 0:
            if bid_v >= ask_v * 1.5: prio += 1
            elif ask_v >= bid_v * 1.5: prio -= 1
        ...
        if "FAKE" in s_bandar or "AWAS" in s_bandar: wpi_text = f"🚫 {wpi_score}% (DISTRIBUSI)"
        elif "SEROK" in s_bandar: wpi_text = f"🎯 {wpi_score}% (BUY WALL)"
        elif wpi_score >= 80: wpi_text = f"🐋 {wpi_score}% (POWER)"
        elif wpi_score <= 30: wpi_text = f"🩸 {wpi_score}% (DUMP)"
        else: wpi_text = f"{wpi_score}%"
        
        h_trd.append({
            "PRIORITY": prio, "RAW_WPI": wpi_score,
            "TICKER": t, "HARGA": f"{int(h):,}".replace(",", "."), 
            "BID [VOL]": bid_str, "OFFER [VOL]": offer_str,
    ...
    df_trd = pd.DataFrame(h_trd).sort_values(["PRIORITY", "RAW_WPI"], ascending=[False, False]).drop(columns=["PRIORITY", "RAW_WPI"]).set_index("TICKER").head(15) if h_trd else pd.DataFrame()
    ...
    if "TRADING" in engine_mode:
        ...
        with tab_t1:
            ...
            def style_t(row):
                ...
                for c, v in row.items():
                    if c in ['BID [VOL]', 'OFFER [VOL]']: stls.append('color:#f8fafc; font-weight:800; text-align:center;')
                    ...
                    elif c == 'BANDARMOLOGI': stls.append('color:#d4af37; font-weight:800;' if 'SEROK' in v else ('color:#f43f5e; font-weight:800;' if 'AWAS' in v or 'DISTRIBUSI' in v else ('color:#00f2fe; font-weight:800;' if 'AKUMULASI' in v else ('color:#10b981; font-weight:800;' if 'MARK-UP' in v else 'color:#64748b;'))))
        ...
        with tab_t2: st.info("SOP REAKTIF v16.5: Mesin kini mengkalkulasi ketebalan antrean Beli vs Jual (Order Book Imbalance). Tabel akan memprioritaskan saham dengan Tembok Beli (Buy Wall) yang kuat untuk Serok Bawah Anti-Boncos.")
