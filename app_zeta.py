...
# ==========================================
CACHE_FILE = "jihan_ghina_saham_cache_v164.json"
...
# ==========================================
st.set_page_config(page_title="JIHAN-GHINA Executive v16.4", page_icon="⚡", layout="wide")
...
def fetch_single_stock(emiten, mode_tf):
    try:
        ...
        # 2. Mendeteksi "Fake Accumulation" 
...
# ENGINE GRAFIK KUARTAL (PLOTLY TERKUNCI & DIPENDEKKAN)
...
def plot_luxury_bar(x_data, y1, y2, name1, name2, color1, color2, title):
    ...
    fig.update_layout(
        height=320, # FITUR BARU: CHART DIBUAT LEBIH PENDEK & PRESISI
        ...
        margin=dict(l=10, r=10, t=35, b=10), # FITUR BARU: MARGIN DIRAPATKAN
        dragmode=False, 
...
def render_cross_validation_ui(active_tickers_tuple, market_climate_mult, is_trading_mode):
    ...
    if active_tickers_tuple:
        safe_key = f"cv_target_v164_{st.session_state.current_tf}_{'TRD' if is_trading_mode else 'INV'}"
...
with st.sidebar:
    ...
    st.markdown("<p style='color: #00f2fe; font-size: 0.65rem; letter-spacing: 2px; margin-bottom: 25px;'>v16.4 PRECISION ENGINE</p>", unsafe_allow_html=True)
...
if not st.session_state.scan_clicked or not st.session_state.raw_stocks:
    st.info("👈 Tekan tombol '🔄 SCAN MARKET' untuk memulai Inisiasi Reactive Engine v16.4.")
else:
    h_trd, h_inv = [], []
    c_val, c_gro, c_div = [], [], []
    ...
    for r in st.session_state.raw_stocks:
        ...
        # LOGIC REKOMENDASI LOT & PRIORITY (REACTIVE SORTING v16.4)
        ...
        h_trd.append({
            "PRIORITY": prio, "RAW_WPI": wpi_score, # Kolom tersembunyi untuk sorting cerdas
    ...
    # FITUR BARU: Tabel Trading Tidak Lagi Disortir Berdasarkan Top Gainer, tapi Berdasarkan Prioritas Serok & Kekuatan WPI
    ...
    if "TRADING" in engine_mode:
        ...
        with tab_t2: st.info("SOP REAKTIF v16.4: Abaikan saham berlabel 'FAKE ACCUMULATION'. Fokus pada saham berlabel '🎯 SEROK BAWAH' untuk menangkap pantulan (rebound) di area support yang kering volume.")
