...
# ==========================================
CACHE_FILE = "jihan_ghina_saham_cache_v172.json"
...
# ==========================================
# 1. KONFIGURASI HALAMAN & COMPACT UI (v17.2)
# ==========================================
st.set_page_config(page_title="JIHAN-GHINA Compact v17.2", page_icon="⚡", layout="wide")
...
<style>
    ...
    .block-container { padding-top: 1rem !important; padding-bottom: 1.5rem !important; padding-left: 0.8rem !important; padding-right: 0.8rem !important; max-width: 100% !important; animation: fadeIn 0.4s ease-in-out; }
    @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
    
    h1 { color: #f8fafc; font-weight: 800; letter-spacing: -0.5px; font-size: 1.3rem !important; margin-bottom: 0; text-shadow: 0 2px 10px rgba(0,242,254,0.15); }
    ...
    /* MODULAR CARD DESIGN - COMPACT */
    .stocksly-card { background: linear-gradient(145deg, rgba(15,23,42,0.85) 0%, rgba(10,15,30,0.95) 100%); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 10px; padding: 12px; box-shadow: 0 4px 15px -5px rgba(0, 0, 0, 0.5); transition: all 0.2s ease; display: flex; flex-direction: column; justify-content: space-between; height: 100%; position: relative; overflow: hidden; }
    .card-title { font-size: 0.65rem; font-weight: 800; color: #64748b; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 8px; display: flex; align-items: center; gap: 4px; }
    
    /* VIP HIGHLIGHT CARDS - COMPACT */
    .vip-card { background: linear-gradient(145deg, rgba(30,41,59,0.7) 0%, rgba(15,23,42,0.9) 100%); backdrop-filter: blur(10px); border: 1px solid rgba(0, 242, 254, 0.3); border-radius: 10px; padding: 12px; position: relative; overflow: hidden; display: flex; flex-direction: column; justify-content: space-between; height: 100%;}
    ...
    .vip-title { font-size: 1.2rem; font-weight: 800; color: #f8fafc; margin: 0;}
    .vip-price { font-size: 1rem; font-weight: 800; margin: 4px 0 8px 0; }
    .vip-badge { background: rgba(0, 242, 254, 0.15); color: #00f2fe; padding: 4px 8px; border-radius: 6px; font-size: 0.6rem; font-weight: 800; display: inline-block; border: 1px solid rgba(0, 242, 254, 0.4);}
    .vip-stat-row { display: flex; justify-content: space-between; margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.05); }
    .vip-stat { text-align: center; }
    .vip-stat-label { font-size: 0.5rem; color: #64748b; font-weight: 800; letter-spacing: 0.5px; text-transform: uppercase; }
    .vip-stat-val { font-size: 0.75rem; font-weight: 800; margin-top: 2px; }
    
    /* SIDEBAR 140PX STRICT */
    section[data-testid="stSidebar"] { width: 140px !important; min-width: 140px !important; max-width: 140px !important; background: linear-gradient(180deg, rgba(2,6,23,0.98) 0%, rgba(15,23,42,0.98) 100%) !important; border-right: 1px solid rgba(255, 255, 255, 0.03); padding-top: 1rem !important;}
    section[data-testid="stSidebar"] .stMarkdown h2 { font-size: 0.9rem !important; margin-bottom: -5px !important; }
    section[data-testid="stSidebar"] p, section[data-testid="stSidebar"] span { font-size: 0.6rem !important; }
    section[data-testid="stSidebar"] label { font-size: 0.55rem !important; font-weight: 700 !important; color: #cbd5e1 !important; }
    section[data-testid="stSidebar"] input { font-size: 0.65rem !important; padding: 4px !important; }
    section[data-testid="stSidebar"] div[data-baseweb="select"] { font-size: 0.65rem !important; }
    
    .stTabs [data-baseweb="tab-list"] { gap: 4px; background-color: rgba(15,23,42,0.4); padding: 4px; border-radius: 8px; margin-bottom: 10px;}
    .stTabs [data-baseweb="tab"] { padding: 6px 10px; border-radius: 6px; font-size:0.7rem; }
    
    .ihsg-box { text-align: right; display: flex; flex-direction: column; justify-content: center; height: 100%; padding: 8px 12px !important; background: linear-gradient(135deg, rgba(15,23,42,0.7) 0%, rgba(30,41,59,0.4) 100%); border-radius: 8px; }
    .ihsg-title { color: #64748b; font-size: 0.55rem; font-weight: 800; letter-spacing: 1px; }
    .ihsg-score { color: #f8fafc; font-size: 1.1rem; font-weight: 800; line-height: 1.1; margin: 2px 0; }
    
    div.stButton > button:first-child { background: linear-gradient(90deg, rgba(30,41,59,0.5) 0%, rgba(15,23,42,0.8) 100%) !important; border: 1px solid rgba(255, 255, 255, 0.1) !important; color: #cbd5e1 !important; border-radius: 6px !important; padding: 4px 8px !important; font-size: 0.65rem !important;}
    
    .stDataFrame { font-size: 10px !important; }
    th.row_heading { color: #00f2fe !important; font-size: 0.75rem !important; }

    .block-container [data-testid="stRadio"] > div[role="radiogroup"] > label { padding: 4px 10px !important; border-radius: 4px !important; }
    .block-container [data-testid="stRadio"] > div[role="radiogroup"] > label p { font-size: 0.65rem !important; }
</style>
...
# ==========================================
QUOTES_DATABASE = [
    {"quote": "Pixel perfect execution.", "author": "Stocks.ly", "theme": "Compact"},
    {"quote": "Ekor lilin di support adalah jejak akumulasi nyata.", "author": "PA", "theme": "Rejection"}
]
...
def render_badges(tickers, hex_color):
    ...
    res = "<div style='display:flex; flex-wrap:wrap; gap:4px; margin-top:6px;'>"
    for t in tickers: res += f"<span style='background:rgba(0,0,0,0.2); border:1px solid {hex_color}40; border-radius:4px; padding:2px 6px; color:{hex_color}; font-size:0.6rem; font-weight:700;'>{t}</span>"
    res += "</div>"
    return res
...
# ==========================================
# 4. COMPACT MODULAR DASHBOARD UI
# ==========================================
def render_cross_validation_ui(active_tickers_tuple, market_climate_mult, is_trading_mode):
    st.markdown("---")
    if active_tickers_tuple:
        safe_key = f"cv_target_v172_{st.session_state.current_tf}_{'TRD' if is_trading_mode else 'INV'}"
        ...
        if r:
            if is_trading_mode:
                ...
                st.markdown(f"""
                <div class="stocksly-card" style="margin-bottom: 15px; border-color: rgba(0, 242, 254, 0.25);">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            <div style="display:flex; align-items:center; gap:6px;">
                                <h2 style="color:#f8fafc; font-size:1.2rem; margin:0;">{emiten_signal}</h2>
                                <span style="background:rgba(0,242,254,0.1); color:#00f2fe; padding:2px 6px; border-radius:4px; font-size:0.55rem; font-weight:800; border:1px solid rgba(0,242,254,0.3);">{sector}</span>
                            </div>
                            <p style="color:#94a3b8; font-size:0.65rem; margin:2px 0 0 0;">{long_name}</p>
                        </div>
                        <div style="text-align:right;">
                            <div style="font-size:0.5rem; color:#64748b; font-weight:800; letter-spacing:0.5px;">WPI SCORE</div>
                            ...
                            <div style="font-size:0.55rem; color:#94a3b8; margin-top:2px;">{s_bandar}</div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                    ...
                    <div class="stocksly-card" style="margin-bottom: 10px;">
                        ...
                        <div style="display:grid; grid-template-columns: repeat(2, 1fr); gap:6px; text-align:center;">
                            <div style="background:rgba(255,255,255,0.02); padding:6px; border-radius:6px; border:1px solid rgba(255,255,255,0.04);">
                                <div style="font-size:0.5rem; color:#64748b; font-weight:800;">LAST</div>
                                <div style="font-size:0.9rem; color:#f8fafc; font-weight:800;">{int(h_tgt):,}</div>
                            </div>
                            <div style="background:rgba(255,255,255,0.02); padding:6px; border-radius:6px; border:1px solid rgba(255,255,255,0.04);">
                                <div style="font-size:0.5rem; color:#64748b; font-weight:800;">ATR</div>
                ...
                with col_g2:
                    ...
                    <div class="stocksly-card" style="margin-bottom: 10px;">
                        ...
                        <div style="text-align:center; background:rgba(0,242,254,0.05); border:1px solid rgba(0,242,254,0.2); border-radius:6px; padding:8px; margin-bottom:6px;">
                            <div style="font-size:1rem; font-weight:800; color:{color};">{sys_rec}</div>
                        </div>
                    ...
                    <div class="stocksly-card" style="margin-bottom: 10px; border-left: 3px solid {'#10b981' if 'SEROK' in setup_grade or 'A+' in setup_grade else '#f43f5e'};">
                        ...
                        <div style="display:grid; grid-template-columns: repeat(2, 1fr); gap:6px; margin-bottom:6px;">
                            <div style="background:rgba(255,255,255,0.02); padding:6px; border-radius:6px; text-align:center;">
                                <div style="font-size:0.5rem; color:#64748b; font-weight:800;">BELI</div>
                                <div style="font-size:0.85rem; color:#00f2fe; font-weight:800;">{a_beli}</div>
                            </div>
                            <div style="background:rgba(255,255,255,0.02); padding:6px; border-radius:6px; text-align:center;">
                                <div style="font-size:0.5rem; color:#64748b; font-weight:800;">CUT</div>
                                <div style="font-size:0.85rem; color:#f43f5e; font-weight:800;">{t_stop}</div>
                            </div>
                        </div>
                        <div style="text-align:center; font-size:0.65rem; color:#cbd5e1; background:rgba(255,255,255,0.02); padding:4px; border-radius:4px;">
                ...
                st.markdown(f"<div style='display:flex; justify-content:space-around; background:rgba(15,23,42,0.8); border:1px solid rgba(255,255,255,0.05); border-radius:8px; padding:10px; margin-bottom:10px;'><div style='text-align:center;'><span style='color:#64748b; font-size:0.55rem; font-weight:700;'>PER</span><br><span style='color:#f8fafc; font-weight:800; font-size:0.9rem;'>{per:.1f}x</span></div><div style='text-align:center;'><span style='color:#64748b; font-size:0.55rem; font-weight:700;'>PBV</span><br><span style='color:#f8fafc; font-weight:800; font-size:0.9rem;'>{pbv:.1f}x</span></div><div style='text-align:center;'><span style='color:#64748b; font-size:0.55rem; font-weight:700;'>YIELD</span><br><span style='color:#10b981; font-weight:800; font-size:0.9rem;'>{yld:.1f}%</span></div></div>", unsafe_allow_html=True)
...
# ==========================================
# 5. SIDEBAR (140px STRICT)
# ==========================================
with st.sidebar:
    st.markdown("<h2 style='color: #f8fafc; font-weight: 800;'>Quantum Matrix</h2>", unsafe_allow_html=True)
    st.markdown("<p style='color: #00f2fe; letter-spacing: 1px; margin-bottom: 15px;'>v17.2 COMPACT</p>", unsafe_allow_html=True)
...
# 6. HEADER DASHBOARD
# ==========================================
st.markdown("<h1>Stocks.ly Compact Dashboard</h1>", unsafe_allow_html=True)

df_ihsg_hist, ihsg_now, ihsg_chg, ihsg_pct = fetch_ihsg_data()

c_h1, c_h2, c_h3 = st.columns([1.5, 1, 1])
with c_h1:
    q = get_quote_of_the_day()
    st.markdown(f"<div style='margin-top:5px;'><span style='color:#00f2fe; font-size:0.55rem; font-weight:800; letter-spacing:1px;'>{q['theme'].upper()}</span><br><span style='color:#cbd5e1; font-size:0.7rem; font-style:italic;'>\"{q['quote']}\"</span></div>", unsafe_allow_html=True)
    upd_time = st.session_state.last_update if st.session_state.last_update else "-"
    st.markdown(f"<div style='margin-top:4px; font-size:0.6rem; color:#64748b;'>Sync: {upd_time}</div>", unsafe_allow_html=True)

with c_h2:
    if ihsg_now:
        w_p, w_g = ("▲", '#10b981') if ihsg_chg >= 0 else ("▼", '#f43f5e')
        st.markdown(f"<div class='ihsg-box' style='border-left:2px solid {w_g};'><span class='ihsg-title'>IHSG</span><span class='ihsg-score'>{ihsg_now:,.0f}</span><span style='color:{w_g}; font-weight:700; font-size:0.65rem;'>{w_p} {ihsg_chg:+,.1f} ({ihsg_pct:+.2f}%)</span></div>", unsafe_allow_html=True)
...
with c_h3:
    if st.session_state.scan_clicked and st.session_state.raw_stocks:
        ...
        st.markdown(f"<div class='ihsg-box' style='border-left:2px solid {c_col};'><span class='ihsg-title'>CLIMATE</span><span class='ihsg-score' style='color:{c_col};'>{c_stat}</span><span style='color:#64748b; font-weight:700; font-size:0.65rem;'>Breadth: {b_pct:.0f}%</span></div>", unsafe_allow_html=True)
    else: c_mult = 1.0

st.markdown("---")

if not st.session_state.scan_clicked or not st.session_state.raw_stocks:
    st.info("👈 Tekan tombol '🔄 SCAN' untuk memulai.")
else:
    h_trd, h_inv = [], []
    c_val, c_gro, c_div = [], [], []
    ...
    if "TRD" in engine_mode:
        tab_t1, tab_t2 = st.tabs(["🚀 ELITE MATRIX", "📜 SOP"])
        with tab_t1:
            st.markdown("<h3 style='font-size: 0.9rem; color:#00f2fe; margin-bottom: 8px;'>👑 Top 3 Elite Picks</h3>", unsafe_allow_html=True)
                        ...
                        <div class="vip-card">
                            <div>
                                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                                    ...
                                    <span style="color:{ret_color}; font-weight:800; font-size:0.8rem;">{ret_sign}{ret:.1f}%</span>
                                </div>
            ...
            st.markdown("<br><h3 style='font-size: 0.9rem; color:#f8fafc; margin-top: 10px;'>🛰️ Top 15 Sinyal</h3>", unsafe_allow_html=True)
        ...
        with tab_t2: st.info("SOP REAKTIF v17.2: Sidebar dilimit ke 140px. UI diperkecil untuk tampilan Pixel-Perfect di Mobile.")
